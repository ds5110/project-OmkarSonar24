...
def get_first_admission_durations(sub_df):
    ...
    df_sequences = (
        sub_df.groupby("person_id")
        .apply(find_consecutive_target_sequences)
    ...
    first_sequences["duration"] = (
        first_sequences["last_end"] - first_sequences["first_start"]
    )
    first_sequences["duration"] = first_sequences.duration.apply(
        lambda x: round(x.total_seconds() / 3600 / 24)
